...
df[numeric].corr(method='spearman') # pearsonr или spearmanr или kendalltau
sns.heatmap(df[numeric].corr(method='spearman'))
```

```
from scipy.stats import pearsonr, spearmanr, kendalltau
r = pearsonr(df['Признак1'], df['Признак2'])
print('Pearson correlation:', r[0], 'p-value:', r[1])
```

👀 ❗❗❗ Так как *p-value* < 0.05 (типичное пороговое значение), то делаем вывод о том, что взаимосвязь (корреляция) между *Признак1* и *Признак2* статистически значима.
